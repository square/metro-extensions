 ...
 * 1. **Real service with @FakeMode check** (params: `serviceCreator`, `isFakeMode`): `if
 *    (isFakeMode) error("No fake service provided for ..."); return serviceCreator.create(...)`
...
private class ContributesServiceIrTransformer(private val pluginContext: IrPluginContext) :
   ...
   * ```
   *
   * The `@FakeMode` check catches cases where fake mode is enabled but no fake service was provided
   * (e.g., a `:fake` module wasn't included in the dependency graph).
   */
  private fun generateRealServiceWithCheckBody(declaration: IrSimpleFunction) {
    val serviceType = declaration.returnType
    val serviceClassSymbol = (serviceType as? IrSimpleType)?.classOrNull ?: return
  ...
  private fun generateRealServiceProviderBody(declaration: IrSimpleFunction) {
    ...
    declaration.body = irBuilder.irBlockBody { +irReturn(createCall) }
  }
  ...
  private fun buildServiceCreatorCall(
    ...
    val serviceType = declaration.returnType
    val serviceClassSymbol = (serviceType as? IrSimpleType)?.classOrNull ?: return null
