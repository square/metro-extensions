 ...
 *
 * When a fake service uses `replaces = [RealService::class]`, this extension resolves the
 * replaced service's `MetroContribution` type and passes it in
 * [MetroContributionExtension.Contribution.replaces] so Metro removes the real service's
 * contribution from the graph.
 */
public class ContributesServiceMetroExtension(private val session: FirSession) :
  ...
  override fun getContributions(
    ...
    return annotatedClasses.mapNotNull { parentSymbol ->
      ...
      // - Cross-module (via hints): keyed by ServiceContribution ClassId
