 ...
 *
 * This is necessary because Metro's predicate-based provider only discovers source declarations
 * and declarations generated by generators that registered the `@ContributesTo` predicate. Our
...
public class ContributesServiceMetroExtension(private val session: FirSession) :
  ...
  /**
   * Build the replaces ClassId list for the contribution. Includes both the origin ClassId
   * (e.g., `MyService`) and its `ServiceContribution` ClassId because Metro keys contributions
   * differently depending on how they were discovered:
